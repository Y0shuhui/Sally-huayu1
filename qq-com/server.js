...
var server = http.createServer(function(request, response){
  ...
  if (path === '/index.html'){
    ...
    response.setHeader('Access-Control-Allow-Origin','http://sally.com:9999');
    response.write(fs.readFileSync('./public/friend.json'));
    response.end();
  } 
  else if(path === '/friends.js') {
    if(request.headers['referer'].indexOf('http://sally.com:9999') === 0){
      response.statusCode = 200;
...
server.listen(port);
console.log('监听 ' + port + ' 成功\n请用在空中转体720度然后用你的小手打开 http://localhost:' + port);
